...
def main(file_path, document, doc_db_path, mtime, _logger):
    ...
    if mime_type in STANDARD_IMAGE_MIME_TYPES:
        ...
        with WandImage(filename=file_path, resolution=300) as img:
            img.format = 'png'
            image_blobs = [img.sequence[i].make_blob() for i in range(len(img.sequence))]
            images = [np.array(preprocess_image(Image.open(io.BytesIO(blob)))) for blob in image_blobs]
    ...
    document["read_text"] = re.sub(r'\s{2,}', ' ', re.sub(r'[^\w\s]+|\s{2,}', ' ', str(read_text))).strip().lower()
