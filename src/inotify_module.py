...
from shared import (
    ...
    validate_and_update_document_paths,
)
...
class EventHandler(pyinotify.ProcessEvent):
    ...
    def record_event(self, path, event_type, move_from_path=None):
        with self.lock:
            ...
            self.timer = threading.Timer(
                self.debounce_delay, self.process_pending_events
            )
            self.timer.start()
    ...
    def process_pending_events(self):
        ...
        if documents_to_delete:
            ...
            logging.info(f"deleted {len(documents_to_delete)} from meili")
        if documents_to_update:
            self.index.update_documents(documents_to_update)
            logging.info(f"upserted {len(documents_to_update)} in meili")
